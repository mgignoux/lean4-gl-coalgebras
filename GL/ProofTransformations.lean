...
structure CutCoalgebra {𝕏 : Split.Proof} (x : 𝕏.X) (τ : 𝕏.X → SplitSequent) where
  X : Type
  α : X → (T x τ).obj X
...
-/
structure CutProofFromPremises {𝕏 : Split.Proof} (x : 𝕏.X) (τ : 𝕏.X → SplitSequent) extends CutCoalgebra x τ where
  -- X : Type
  -- α : X → (T x τ).obj X
...
(box_prop : ∀ x, (r 𝕏.α x).isBox →  -- on every path from the root to non-ax leaves there is a box node
  ...
  { X := (y : 𝕏.X) × (PartialProof y).X
    ...
    h := by -- DONT DELETE THE PROOF BELOW, ITS COMMENTED FOR NOW BECAUSE ITS SO SLOW
