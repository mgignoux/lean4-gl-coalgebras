...
inductive RuleApp (P : List Sequent)
  | pre : (Δ : Sequent) → (Δ ∈ P) → RuleApp P
  | cut : (Δ : Sequent) → (A : Formula) → RuleApp P
  | wk : (Δ : Sequent) → (A : Formula) → (A ∈ Δ) → RuleApp P
  | skp : (Δ : Sequent) → RuleApp P
  | top : (Δ : Sequent) → ⊤ ∈ Δ → RuleApp P
  | ax : (Δ : Sequent) → (n : Nat) → (at n ∈ Δ ∧ na n ∈ Δ) → RuleApp P
  | and : (Δ : Sequent) → (A : Formula) → (B : Formula) → (A & B) ∈ Δ → RuleApp P
  | or : (Δ : Sequent) → (A : Formula) → (B : Formula) → (A v B) ∈ Δ → RuleApp P
  | box : (Δ : Sequent) → (A : Formula) → (□ A) ∈ Δ → RuleApp P

def fₚ {P : List Sequent} : RuleApp P → Sequent
  | RuleApp.pre Δ _ => Δ
  | RuleApp.cut _ _ => ∅
  | RuleApp.wk _ A _ => {A}
  | RuleApp.skp _ => {}
...
def f {P : List Sequent} : RuleApp P → Sequent
  | RuleApp.pre Δ _ => Δ
  | RuleApp.cut Δ _ => Δ
  | RuleApp.wk Δ _ _ => Δ
  | RuleApp.skp Δ => Δ
...
def fₙ {P : List Sequent} : RuleApp P → Sequent := fun r ↦ f r \ fₚ r
...
structure CutProofFromPremises (P : List Sequent) where
  ...
  h : ∀ (x : X), match r α x with
    ...
    | RuleApp.box _ A _ => (p α x).map (fun x ↦ f (r α x)) = [(fₙ (r α x)).D ∪ {A}]


def CutProofFromPremises.Proves {P : List Sequent} (𝕏 : CutProofFromPremises P) (Δ : Sequent) : Prop := ∃ x : 𝕏.X, f (r 𝕏.α x) = Δ
