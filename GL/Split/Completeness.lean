...
lemma rewind_history_one_step_in_cone {Γ} (g : coalgebraGame.Pos) (h : coalgebraGame.turn g = Prover ∧ g.2.2 ≠ ∅ ∨ coalgebraGame.turn g = Builder ∧ g.2.1 ≠ ∅) -- (h : winning strat (startPos Γ))  (in_cone : inMyCone strat (startPos Γ) g)
...
lemma rewind_history_in_cone {Γ} (g : coalgebraGame.Pos) (n : Fin ((if coalgebraGame.turn g = Prover then min (2 * g.2.1.length + 1) (2 * g.2.2.length) else min (2 * g.2.1.length) (2 * g.2.2.length + 1)) + 1) )
  (strat : Strategy coalgebraGame Prover) (in_cone : inMyCone strat (startPos Γ) g)
...
def next_next {Γ Δ : SplitSequent} {strat : Strategy coalgebraGame Prover} (g : proof_type Γ strat)
  (h : winning strat (startPos Γ)) (nrep : Δ ∉ g.1.2.1) (pos : Δ ∈ (builder_RuleApp g.1 g.2.2).splitSequents) : proof_type Γ strat :=
...
lemma next_next_cor {Γ Δ : SplitSequent} {strat : Strategy coalgebraGame Prover} (g : proof_type Γ strat)
  (h : winning strat (startPos Γ)) (nrep : Δ ∉ g.1.2.1) (pos : Δ ∈ (builder_RuleApp g.1 g.2.2).splitSequents) :
...
lemma history_length_in_cone {Γ : SplitSequent} (strat : Strategy coalgebraGame Prover) (g : coalgebraGame.Pos)
   ...
   rewind_history_in_cone g.1 ⟨(2 * (Fin.find _ (List.mem_iff_get.1 rep)).1), _⟩ strat g.2.1,
    by
      have := @rewind_turn g.1 ⟨(2 * (Fin.find _ (List.mem_iff_get.1 rep)).1), by
        ...
        simp [g.2.2] at *
        have := Fin.find _ (List.mem_iff_get.1 rep)
        grind⟩
      simp [g.2.2] at this
      convert this
      simp [rep_pos]⟩
...
def builder_move_premises {Γ : SplitSequent} {strat : Strategy coalgebraGame Prover} (g : proof_type Γ strat)
        ...
        if rep1 : (Δ \ {Sum.inl (φ1 & φ2)}) ∪ {Sum.inl φ1} ∈ Γs
          then
            if rep2 : (Δ \ {Sum.inl (φ1 & φ2)}) ∪ {Sum.inl φ2} ∈ Γs
              ...
              else [rep_next Γ g (by convert rep1; grind), next_next g h (by convert rep2; grind) (by subst g_def; simp [RuleApp.splitSequents, builder_RuleApp])]
          else
            if rep2 : (Δ \ {Sum.inl (φ1 & φ2)}) ∪ {Sum.inl φ2} ∈ Γs
              then [next_next g h (by convert rep1; grind) (by subst g_def; simp [RuleApp.splitSequents, builder_RuleApp]), rep_next Γ g (by convert rep2; grind)]
      ...
      · subst g_def
        ...
        case andₗ Δ φ1 φ2 φ_in =>
          ...
          case pos rep1 =>
            ...
            case pos rep2 =>
              ...
              constructor
              · exact rep_next_cor Γ ⟨⟨Sum.inr (RuleApp.andₗ Δ φ1 φ2 φ_in), Γs, Rs⟩, in_cone, b_move⟩
                  (by simp only [rep1])
              · exact rep_next_cor Γ ⟨⟨Sum.inr (RuleApp.andₗ Δ φ1 φ2 φ_in), Γs, Rs⟩, in_cone, b_move⟩
                  (by simp only [rep2])
            case neg nrep2 =>
        ...
        case andᵣ Δ φ1 φ2 φ_in =>
          ...
          case pos rep1 =>
            ...
            case pos rep2 =>
              simp only [rep1, rep2, ↓reduceDIte, List.cons.injEq, and_true, ↓existsAndEq, true_and, rep_next, fₙ_alternate]
              constructor
              · exact rep_next_cor Γ ⟨⟨Sum.inr (RuleApp.andᵣ Δ φ1 φ2 φ_in), Γs, Rs⟩, in_cone, b_move⟩
                  (by simp only [rep1])
              · exact rep_next_cor Γ ⟨⟨Sum.inr (RuleApp.andᵣ Δ φ1 φ2 φ_in), Γs, Rs⟩, in_cone, b_move⟩
                  (by simp only [rep2])
            case neg nrep2 =>
        ...
        case orₗ Δ φ1 φ2 φ_in =>
          ...
          case pos rep =>
            ...
            simp only [rep_next]
            exact rep_next_cor Γ ⟨⟨Sum.inr (RuleApp.orₗ Δ φ1 φ2 φ_in), Γs, Rs⟩, in_cone, b_move⟩
              (by simp only [rep])
          case neg nrep =>
        ...
        case orᵣ Δ φ1 φ2 φ_in =>
          ...
          case pos rep =>
            simp only [rep, ↓reduceDIte, List.cons.injEq, and_true, exists_eq_left']
            simp only [rep_next]
            exact rep_next_cor Γ ⟨⟨Sum.inr (RuleApp.orᵣ Δ φ1 φ2 φ_in), Γs, Rs⟩, in_cone, b_move⟩
              (by simp only [rep])
          case neg nrep =>
            simp only [nrep, ↓reduceDIte, List.cons.injEq, and_true, exists_eq_left']
            simp only [next_next, fₙ_alternate]
        ...
        case boxₗ Δ φ1 φ_in =>
          ...
          case pos rep =>
            simp only [rep, ↓reduceDIte, List.cons.injEq, and_true, exists_eq_left']
            simp only [rep_next]
            exact rep_next_cor Γ ⟨⟨Sum.inr (RuleApp.boxₗ Δ φ1 φ_in), Γs, Rs⟩, in_cone, b_move⟩
              (by simp only [rep])
          case neg nrep =>
            simp only [nrep, ↓reduceDIte, List.cons.injEq, and_true, exists_eq_left']
            simp only [next_next, fₙ_alternate]
        ...
        case boxᵣ Δ φ1 φ_in =>
          ...
          case pos rep =>
            simp only [rep, ↓reduceDIte, List.cons.injEq, and_true, exists_eq_left']
            simp only [rep_next]
            exact rep_next_cor Γ ⟨⟨Sum.inr (RuleApp.boxᵣ Δ φ1 φ_in), Γs, Rs⟩, in_cone, b_move⟩
              (by simp only [rep])
          case neg nrep =>
            simp only [nrep, ↓reduceDIte, List.cons.injEq, and_true, exists_eq_left']
            simp only [next_next, fₙ_alternate]
        ...
        all_goals
          ...
          rfl}
  have turn_P : coalgebraGame.turn (startPos Γ) = Prover := by simp [coalgebraGame]
...
@[simp]
def MaximalPath.last {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder} : MaximalPath Γ strat → coalgebraGame.Pos :=
  fun π => π.list.getLast π.ne

@[simp]
def MaximalPath.first {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder} : MaximalPath Γ strat → coalgebraGame.Pos :=
  fun π => π.list.head π.ne

lemma maximal_path_starts_in_prover_turn {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder}
...
lemma maximal_path_ends_in_prover_turn {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder} (h : winning strat (startPos Γ))
  (π : MaximalPath Γ strat) :
...
lemma make_path_is_in_cone (Δ : SplitSequent) (strat : Strategy coalgebraGame Builder) (g : coalgebraGame.Pos) (in_cone : inMyCone strat (startPos Δ) g) (h : winning strat (startPos Δ))
...
lemma always_exists_maximal_path_from_root_or_after (Γ : SplitSequent) (strat : Strategy coalgebraGame Builder)
...
def last_SplitSequent {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder} (h : winning strat (startPos Γ))
  : MaximalPath Γ strat → SplitSequent := fun π ↦
...
lemma maximal_path_refl_trans_gen
...
lemma move_from_last_implies_box {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder} (π : MaximalPath Γ strat) :
...
lemma diamond_in_last_of_diamond_in_first {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder} (h : winning strat (startPos Γ)) :
∀ π : MaximalPath Γ strat, ∀ φ (i : ℕ) (lt : i < π.list.length) helper (ps),
  ◇ φ ∈ (prover_SplitSequent ((π.list)[π.list.length - i - 1]'helper) ps).toSequent → ◇ φ ∈ (last_SplitSequent h π).toSequent := by
...
lemma formula_in_successor_of_diamond_formula_in {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder}
 ...
 lemma diamond_in_path_of_diamond_formula_in {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder}
...
lemma formula_in_path_of_diamond_formula_in {Γ : SplitSequent} {strat : Strategy coalgebraGame Builder}
  (h : winning strat (startPos Γ)) {π ρ : MaximalPath Γ strat} (π_ρ : Relation.TransGen (path_relation Γ strat) π ρ) :
  ∀ φ, ◇ φ ∈ (last_SplitSequent h π).toSequent → φ ∈ (first_SplitSequent ρ).toSequent := by
  intro φ φ_in
  cases π_ρ
...
lemma builder_win_strong {Δ : SplitSequent} (strat : Strategy coalgebraGame Builder) (h : winning strat (startPos Δ))
  (π : MaximalPath Δ strat) (φ) (i : ℕ) (lt : i < π.list.length) helper (ps) :
  φ ∈ (prover_SplitSequent ((π.list)[π.list.length - i - 1]'helper) ps).toSequent → ¬ evaluate (game_b_model Δ h, π) φ := by
  ...
  · cases i
    case zero =>
      ...
      case atom n =>
        simp [game_b_model, SplitSequent.toSequent, -not_or]
        left
        convert φ_in
        simp [prover_SplitSequent, last_SplitSequent]
        grind
      case negAtom n =>
        simp [game_b_model, SplitSequent.toSequent]
        constructor
        · intro nφ_in
      ...
      case diamond φ =>
        simp
        intro ρ π_ρ
        apply builder_win_strong strat h ρ φ (ρ.list.length - 1) --- termination
      ...
      case box φ =>
        ...
        have next_next_in_cone : inMyCone strat (startPos Δ) next_next_move.1 := by
    ...
    case succ i =>
      ...
      case succ i =>
        ...
        simp [←eq] at u₂_def
        have eq_helper : prover_SplitSequent π[π.length - i - 1] P_turn = Γ' := by grind [prover_SplitSequent]
        by_cases Sum.inl φ ∈ Γ'
        case pos φ_in =>
          exact builder_win_strong strat h ⟨π, ne, chain, max, head_cases, in_cone⟩ φ i (by grind) (by grind) P_turn (by simp [SplitSequent.toSequent]; exact Or.inl (eq_helper ▸ φ_in))
        case neg nφ_in =>
  ...
  · cases i
    ...
    case succ i =>
      ...
      case succ i =>
        ...
        case pos φ_in =>
          exact builder_win_strong strat h ⟨π, ne, chain, max, head_cases, in_cone⟩ φ i (by grind) (by grind) P_turn (by simp [SplitSequent.toSequent]; exact Or.inr (eq_helper ▸ φ_in))
        case neg nφ_in =>
          cases R <;> simp [RuleApp.splitSequents] at Γ'_R
          case andᵣ Δ ψ₁ ψ₂ in_Δ _ =>
            have ⟨eq1, eq2⟩ : φ = (ψ₁ & ψ₂) ∧ Γ = Δ := by
...
lemma single_preserves_equiv (n : Nat) (φ ψ χ : Formula) (equiv : φ ≅ ψ) : single n χ φ ≅ single n χ ψ :=
